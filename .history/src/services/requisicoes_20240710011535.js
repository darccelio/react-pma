...
//           return `Erro na requisição: ${errorData.message}`
//         }
...
//         setEr(error.message || 'Erro na requisição')
//       }
//     }
...
// Exemplo de uma função que faz uma chamada de API sem usar Hooks diretamente
async function GetCategory() {
  try {
    const response = await fetch(URL);
    const data = await response.json();
    
    console.log(`FETCH DATA: ${data.categories[0].id}}`);
    return data;
  } catch (error) {
